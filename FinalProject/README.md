...
> - 주요 상세 내용은 접혀있으므로, **3번(CORE)과 4번(DEEP DIVE) 섹션** 확인을 권장드립니다.
...
- **📊 ERD (Entity Relationship Diagram)**
  ...
  - 👉 **[전체 DB 설계도 (erd.md) 보러가기](./docs/erd.md)**

- **🗺️ IA (Information Architecture)**
  - GNB 중심의 서비스 계층 및 페이지별 접근 권한(Role) 정의
  - 👉 **[사이트 구조도 (ia.md) 보러가기](./docs/ia.md)**

- **🌊 User Flow**
  - 핵심 비즈니스 로직(경로 계산 ➡️ 탄소 산출 ➡️ 리워드)의 흐름 및 예외 처리
  - 👉 **[유저 행동 흐름 (userflow.md) 보러가기](./docs/userflow.md)**

- **📒 DB Structure**
  - 테이블별 제약조건, 데이터 타입 선정 근거 및 인덱스 전략
  - 👉 **[데이터 명세서 (db_structure.md) 보러가기](./docs/db_structure.md)**
