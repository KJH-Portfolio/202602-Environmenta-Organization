...
**💡 8. 개인 주요 기능 요약 (본인 담당)**
- 💬 **실시간 메시징 (Chat)**
  - 채팅방 개설, 참여, 실시간 채팅 전송 및 전역 알림 수신
- 🌤️ **AI 환경 비서 및 환경 데이터 연동 (Weather & News)**
  - 공공데이터(미세먼지, 자외선) 통합 조회 및 글로벌 에코 뉴스 제공
  - 날씨 데이터 기반 맞춤형 환경 실천 조언(비서 멘트)
- 🔐 **보안 및 시스템 인프라 (Security & Configs)**
  - `JwtAuthenticationFilter`를 이용한 공통 토큰 검증, 전역 `WebConfig`, CORS 및 `WebSocketConfig` 세팅
